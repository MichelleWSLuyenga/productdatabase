	...
	<div class="table table-sm">
		<form method="post" name="intable">
			<table class="table table-hover table-bordered" id="table">
				<thead class="thead-light">
					<tr>
						...
						<?php
							if($_SESSION['permission']['edit_warranty'] == '1' || $_SESSION['permission']['delete_warranty'] == '1')
							{ 
						?>
				...
				include('PHP/connect.php');
					$sql = "SELECT `warranty`.*, count(`repair_item`.`warranty_type`) AS exist FROM `warranty` LEFT JOIN `repair_item` ON `warranty`.`warranty_id` = `repair_item`.`warranty_type` GROUP BY `warranty`.`warranty_id`;";
					...
					while($row = mysqli_fetch_array($result))
					{
						...
						if($exist == "0")
						{
							...
							elseif($_SESSION['permission']['edit_warranty'] == '1' && $_SESSION['permission']['delete_warranty'] == '0')
							{
								echo "<td>
								<center>
									<input type='submit' value='edit' style='display:none' class='".$row[0]." btn btn-primary' onclick='frm_sbmt(1)'>
							...
							elseif($_SESSION['permission']['edit_warranty'] == '0' && $_SESSION['permission']['delete_warranty'] == '1')
							{
			...
			<input type="text" id="war_id" name="war_id" required="required" style="display:none">
		</form>
	</div>
...
<script language="javascript">
	...
	for(var i=0; i<table.rows.length; i++)
	{
		table.rows[i].onclick = function()
		{
				...
				document.getElementById("war_id").value = this.cells[0].innerHTML;
				var war_id = document.getElementById("war_id").value;
				var x = table.getElementsByClassName(war_id);
	...
	function validate()
	{
		if(document.getElementById("war_id").value == "")
		{
	...
	function frm_sbmt(num)
	{
		if(num == 1)
		{
			document.intable.action = "edit_warranty.php";
